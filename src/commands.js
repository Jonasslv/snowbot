...
const { geticeQueenInfo, geticeQueenTVL,
    getSnowglobesPool, getSnowglobesTVL, getSnobCircSupply } = require('./abicalls.js');
...
async function runHarvesterOutOfGas(client,harvestStatus) {
  const settings = JSON.parse(readFileSync('./settings.json'));
  const channelHarvester = await client.channels.fetch(settings.channelHarvesterMSG);
  const embedObject = {
    Title: 'Running Out of Gas to Harvest',
    Color: Constants.snowballColor,
    Description: `**AVAX Balance on the Harvester:** ${harvestStatus.AVAXQty.toFixed(2)}`,
  };
  const embedMSGHarvester = makeEmbed(embedObject);

  channelHarvester.send(embedMSGHarvester);
}
...
module.exports = {
  ...
  runHarvesterOutOfGas:runHarvesterOutOfGas
}
